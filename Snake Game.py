...
snake = [(400, 400), (380, 400), (360, 400)] # The snake is a list of tuples, 1st tuple is the head and the other 2 are the body
direction = "RIGHT" # This is the current direction of the snake
change_to = direction # This stores the next direction

# Rects
play_button = pygame.Rect(346, 264, 112, 32) # These are the play  button co ordinates
difficulty_button = pygame.Rect(278, 465, 263, 30) # These are the difficulty button co ordinates
quit_button = pygame.Rect(345, 664, 109, 32) # There are the quid button co ordinates
...
# Screen and colours
WIDTH, HEIGHT = 800, 800 # This is the window size
...
while running: # This is the beginning of the actual loop - running is always True
    ...
    for event in pygame.event.get(): # This is the beginning of the loop that says if x happens do y
        ...
        # Moving the snake
            ...
            if menu_run: # This is the menu loop, it is also the loop that origninally starts due to it being True initially in the flags section
                if play_button.collidepoint(event.pos): # If the mouse is clicked on the play button (established in the rects section)
                    print("Play pressed") # This was for checking if the button was recognised as pressed before there were menu changes
                ...
                elif difficulty_button.collidepoint(event.pos): # If the mouse is clicked on the difficulty button (established in the rects section)
                    print("Difficulty pressed") # This was for checking if the button was recognised as pressed before there were menu changes
                    menu_run = False # Menu loop ends
                    difficulty_run = True # Difficulty menu loop starts
                    game_run = False # Playing loop ends
                elif quit_button.collidepoint(event.pos): # If the mouse is clicked on the play button (established in the rects section)
                    running = False # Whole game loop ends
            elif difficulty_run: # If the difficulty menu loop is active
                if easy_button.collidepoint(event.pos): #  # If the mouse is clicked on the easy button (established in the rects section)
                ...
                elif medium_button.collidepoint(event.pos): # If the mouse is clicked on the medium button (established in the rects section)
                ...
                elif hard_button.collidepoint(event.pos): # If the mouse is clicked on the hard button (established in the rects section)
                    selected_difficulty = "Hard"  # The difficulty is set to hard
                    difficulty_run = False # Difficulty menu loop ends
                    menu_run = True # Menu loop starts again
                    snake_speed = 18 # This is the snakes speed in the hard difficulty 
                elif back_button.collidepoint(event.pos): # If the mouse is clicked on the back button (established in the rects section)
                    difficulty_run = False  # Difficulty menu loop ends
                    menu_run = True # Menu loop starts again
    ...
    if menu_run: # If the menu loop is running
        ...
        write_text("PLAY", font, text_colour, 345, 250) #Write PLAY in the standard font (established in the fonts section)
        write_text("DIFFICULTY", font, text_colour, 275, 450) #Write DIFFICULTY in the standard font (established in the fonts section)
        write_text("QUIT", font, text_colour, 345, 650) #Write QUIT in the standard font (established in the fonts section)
        ...
        write_text("DIFFICULTY", header, text_colour, 150, 50) #Write DIFFICULTY in the header font (established in the fonts section)
        write_text("EASY", font, text_colour, 345, 240) #Write EASY in the standard font (established in the fonts section
        write_text("MEDIUM", font, text_colour, 315, 360) #Write MEDIUM in the standard font (established in the fonts section
        write_text("HARD", font, text_colour, 345, 480) #Write HARD in the standard font (established in the fonts section
        write_text("BACK", font, text_colour, 345, 600) #Write BACK in the standard font (established in the fonts section
